...
def decide_and_execute(context: dict):
    ...
    # ==========================================
    # PATH B: ALGORAND DAPP (TestNet + Amplify)
    # ==========================================
    if project_type == "algorand_dapp" or review.get("is_algorand_dapp"):
        print("[AI] Decision: Algorand dApp detected -> Contract + Frontend Deploy")

        project_path = context.get("project_path", "")
        base_name = context.get("filename", "dapp").replace(".zip", "").replace(".", "-").split("/")[-1]
        app_name = f"depro-{base_name}"

        result = call_mcp_tool("deploy_algorand_dapp", {
